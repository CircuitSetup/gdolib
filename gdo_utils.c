...
const char* gdo_door_state_str[] = {
    "Unknown", "Open", "Closed", "Stopped", "Opening", "Closing",
};
...
const char* gdo_paired_device_type_str[] = {
    "All", "Remote", "Keypad", "Wall Control", "Accessory", "Unknown",
};
...
const char* gdo_door_action_str[] = {
    "Close", "Open", "Toggle", "Stop", "Unknown",
};
...
void print_buffer(gdo_protocol_type_t protocol, uint8_t* buf, bool is_tx) {
    if (protocol == GDO_PROTOCOL_DRY_CONTACT) {
        ...
        ESP_LOGV(TAG,
                 ...
                 is_tx ? "TX" : "RX", buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6],
                 buf[7], buf[8], buf[9], buf[10], buf[11], buf[12], buf[13], buf[14], buf[15],
