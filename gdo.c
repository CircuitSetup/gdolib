...
esp_err_t gdo_init(const gdo_config_t *config) {
  ...
  if (g_status.obst_test_pulse_timer_active) {
    ...
    err = esp_timer_start_periodic(obst_test_pulse_timer,
                                   g_status.obst_test_pulse_timer_usecs);
    if (err != ESP_OK) {
      return err;
    }
  }
  ...
  if (g_status.protocol == GDO_PROTOCOL_DRY_CONTACT) {
    ...
    gpio_config_t io_conf = {
        .pin_bit_mask = (1ULL << g_config.uart_tx_pin) |
                        ((g_config.dc_discrete_open_pin)
                             ? (1ULL << g_config.dc_discrete_open_pin)
                             : 0) |
                        ((g_config.dc_discrete_close_pin)
                             ? (1ULL << g_config.dc_discrete_close_pin)
                             : 0),
        .mode = GPIO_MODE_OUTPUT,
        .pull_up_en = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
    ...
    ESP_LOGI(TAG,
             "Using software emulation of serial port instead of h/w UART");
  }
...
esp_err_t gdo_start(gdo_event_callback_t event_callback, void *user_arg) {
  ...
  if (g_status.protocol != GDO_PROTOCOL_DRY_CONTACT) {
    ...
    if (g_config.use_sw_serial) {
      ...
      esp_err_t err =
          serial_start(gdo_event_queue, &serial_tx_buffer, &serial_task_handle,
  ...
  if (g_config.dc_open_pin > 0) {
    ...
    if (xTaskCreatePinnedToCore(
            gdo_contact_task, "gdo_open_ISR", 4096, &info, 16,
            &gdo_contact_task_handle[GDO_CONTACT_DOOR_OPEN - 1], 1) != pdPASS)
#endif
    {
      return ESP_ERR_NO_MEM;
    }
...
#else
    if (xTaskCreatePinnedToCore(
            gdo_contact_task, "gdo_close_ISR", 4096, &info, 16,
            &gdo_contact_task_handle[GDO_CONTACT_DOOR_CLOSE - 1], 1) != pdPASS)
#endif
    {
      return ESP_ERR_NO_MEM;
    }
...
esp_err_t gdo_door_close(void) {
  ...
  if (g_status.door == GDO_DOOR_STATE_CLOSING ||
    ...
    ESP_LOGI(TAG,
             "Door already closed or closing, ignore request to close door");
    return ESP_OK;
  }
...
static void obst_timer_cb(void *arg) {
  ...
  if (stats->count >= 1) {
    obs_state = GDO_OBSTRUCTION_STATE_CLEAR;
             ...
             (micros_now - stats->sleep_micros < 750000)) {
    obs_state = GDO_OBSTRUCTION_STATE_CLEAR;
  } else {
...
static esp_err_t transmit_packet_sw(uint8_t *packet) {
  ...
  if (!xMessageBufferSend(serial_tx_buffer, &tx_packet, sizeof(tx_packet),
                          portMAX_DELAY)) {
    ...
    if (xQueueReceive(gdo_event_queue, (void *)&event,
      ...
      case SERIAL_EVENT_DATA: {
        if (!g_status.protocol) {
          ...
          if (rx_packet_size == 2) {
            ESP_LOGD(TAG, "Received %u bytes, using protocol V1",
                     rx_packet_size);
      ...
      case UART_DATA: {
        ...
        if (!g_status.protocol) {
          if (rx_packet_size == 2) {
            ...
            g_status.protocol = GDO_PROTOCOL_SEC_PLUS_V1;
          } else if (rx_packet_size == 20 || rx_packet_size == 19 ||
                     rx_pending) {
            ESP_LOGD(TAG, "Received %u bytes, using protocol V2",
                     rx_packet_size);
            g_status.protocol = GDO_PROTOCOL_SEC_PLUS_V2;
        ...
        if (g_status.protocol == GDO_PROTOCOL_SEC_PLUS_V2) {
          if (!rx_pending) {
            ...
            ESP_LOGI(
                TAG, "Unexpected data; received %u bytes, %s", rx_packet_size,
          ...
          if (rx_packet_size < GDO_PACKET_SIZE) {
            ESP_LOGW(TAG, "Ignoring RX packet size %u, pending: %u",
...
update_obstruction_state(gdo_obstruction_state_t obstruction_state) {
  if (obstruction_state != g_status.obstruction) {
    ...
    ESP_LOGV(TAG, "Obstruction state: %s",
             gdo_obstruction_state_str[obstruction_state]);
  }
}
...
static void IRAM_ATTR gdo_contact_isr_handler(void *arg) {
  ...
  if (info->inDebounce) {
    ...
    xTaskNotifyFromISR(info->notifyTask, 0, eNoAction,
...
static void contact_debounce_timer_cb(void *arg) {
  ...
  uint32_t bounceDuration =
      (int32_t)(info->countTimestamp - info->levelTimestamp);
  if (pinLevel == info->level) {
    ...
    ESP_LOGD(TAG,
             ...
             "(%ld.%03ldms)",
             info->pin,
             (info->pin == g_config.dc_open_pin) ? "open sensor"
                                                 : "closed sensor",
    ...
    ESP_LOGD(TAG,
             ...
             (info->pin == g_config.dc_open_pin) ? "open sensor"
                                                 : "closed sensor",
             g_config.dc_debounce_ms, count, bounceDuration / 1000UL,
             bounceDuration % 1000UL);
...
static void gdo_contact_task(void *arg) {
  ...
  ESP_LOGI(
      TAG, "Initialize dry contact ISR and Task for pin: %d (%s)", info->pin,
  ...
  err = gpio_config(&io_conf);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to configure GPIO for pin %d, terminating task",
             info->pin);
    return;
  }
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG,
             "Failed to install ISR handler for GPIO pin %d, terminating task",
             info->pin);
    return;
  }
  ...
  for (;;) {
    ...
    ESP_LOGD(TAG, "Wake up for GPIO pin %d (%s), level is %d", info->pin,
             ...
             info->level);
    esp_timer_stop(
        info->debounceTimer); // Just to be super safe, should not be active.
    esp_timer_start_once(info->debounceTimer, g_config.dc_debounce_ms * 1000);
  }
}
