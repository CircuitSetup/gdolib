...
esp_err_t gdo_init(const gdo_config_t* config) {
    ...
    if (!config || config->uart_num >= UART_NUM_MAX || config->uart_tx_pin >= GPIO_NUM_MAX ||
        config->uart_rx_pin >= GPIO_NUM_MAX || config->obst_in_pin >= GPIO_NUM_MAX ||
        config->obst_tp_pin >= GPIO_NUM_MAX || config->dc_open_pin >= GPIO_NUM_MAX ||
        config->dc_close_pin >= GPIO_NUM_MAX || config->dc_discrete_open_pin >= GPIO_NUM_MAX ||
    ...
    if ((g_config.obst_in_pin > 0) && !g_config.obst_from_status) {
        ...
        ESP_LOGE(TAG, "Failed to initialize GDOLIB... dry contact protocol "
                      "requires obstruction sensor GPIO pin");
        return ESP_FAIL;
    ...
    if (g_status.protocol == GDO_PROTOCOL_DRY_CONTACT) {
        ...
        if (g_config.invert_uart) {
            err =
                uart_set_line_inverse(g_config.uart_num, UART_SIGNAL_RXD_INV | UART_SIGNAL_TXD_INV);
            if (err != ESP_OK) {
                return err;
            }
        }

        err = uart_set_pin(g_config.uart_num, g_config.uart_tx_pin, g_config.uart_rx_pin,
...
esp_err_t gdo_start(gdo_event_callback_t event_callback, void* user_arg) {
    ...
    if (g_status.protocol != GDO_PROTOCOL_DRY_CONTACT) {
        ...
        if (g_config.use_sw_serial) {
            ...
            esp_err_t err = serial_start(gdo_event_queue, &serial_tx_buffer, &serial_task_handle,
            ...
            err =
                uart_driver_install(g_config.uart_num, RX_BUFFER_SIZE, 0, 32, &gdo_event_queue, 0);
            if (err != ESP_OK) {
                return err;
            }
    ...
    if (g_config.dc_open_pin > 0) {
        // Test for > 0 even though zero is valid GPIO pin, but for GDO use case, we
        // require pin to be > 0
        static gdo_contact_t info;
        info.contact = GDO_CONTACT_DOOR_OPEN;
        info.pin = g_config.dc_open_pin;
        // Medium high priority as it needs to handle in real time, pin to CPU 1 so
        // does not share with HomeKit/WiFi/mdns/etc.
#ifdef CONFIG_FREERTOS_UNICORE
        if (xTaskCreate(gdo_contact_task, "gdo_open_ISR", 4096, &info, 16,
                        &gdo_contact_task_handle[GDO_CONTACT_DOOR_OPEN - 1]) != pdPASS)
#else
        if (xTaskCreatePinnedToCore(gdo_contact_task, "gdo_open_ISR", 4096, &info, 16,
                                    &gdo_contact_task_handle[GDO_CONTACT_DOOR_OPEN - 1],
                                    1) != pdPASS)
#endif
        {
            return ESP_ERR_NO_MEM;
    ...
    if (g_config.dc_close_pin > 0) {
        ...
        // does not share with HomeKit/WiFi/mdns/etc.
#ifdef CONFIG_FREERTOS_UNICORE
        if (xTaskCreate(gdo_contact_task, "gdo_close_ISR", 4096, &info, 16,
                        &gdo_contact_task_handle[GDO_CONTACT_DOOR_CLOSE - 1]) != pdPASS)
#else
        if (xTaskCreatePinnedToCore(gdo_contact_task, "gdo_close_ISR", 4096, &info, 16,
                                    &gdo_contact_task_handle[GDO_CONTACT_DOOR_CLOSE - 1],
                                    1) != pdPASS)
#endif
        {
            return ESP_ERR_NO_MEM;
    ...
    // does not share with HomeKit/WiFi/mdns/etc.
#ifdef CONFIG_FREERTOS_UNICORE
    if (xTaskCreate(gdo_main_task, "gdo_main_task", 4096, NULL, 15, &gdo_main_task_handle) !=
        pdPASS)
#else
...
esp_err_t gdo_sync(void) {
    if (g_status.protocol == GDO_PROTOCOL_DRY_CONTACT) {
        // dry contact cannot do a sync, door status remains unknown until some
        // activity
...
esp_err_t gdo_light_on_check(bool check) {
    ...
    if (g_status.protocol & GDO_PROTOCOL_SEC_PLUS_V1) {
        ...
        if (!check || g_status.light != GDO_LIGHT_STATE_ON) {
            ...
            // No need to explicitly update status, wall panel (or emulation) is doing
            // that constantly
        }
    } else {
        ...
        if (check && err == ESP_OK) {
            // retrieve door (light) status if requested. This is optional because it
            // is slow
            get_status();
        }
    }
...
esp_err_t gdo_clear_paired_devices(gdo_paired_device_type_t type) {
    ...
    if (type == GDO_PAIRED_DEVICE_TYPE_ALL) {
        err =
            queue_command(GDO_CMD_CLEAR_PAIRED_DEVICES, (GDO_PAIRED_DEVICE_TYPE_REMOTE - 1), 0, 0);
        if (err != ESP_OK) {
            return err;
        }

        err =
            queue_command(GDO_CMD_CLEAR_PAIRED_DEVICES, (GDO_PAIRED_DEVICE_TYPE_KEYPAD - 1), 0, 0);
        if (err != ESP_OK) {
            return err;
        }

        err = queue_command(GDO_CMD_CLEAR_PAIRED_DEVICES, (GDO_PAIRED_DEVICE_TYPE_WALL_CONTROL - 1),
                            0, 0);
        if (err != ESP_OK) {
            return err;
        }

        err = queue_command(GDO_CMD_CLEAR_PAIRED_DEVICES, (GDO_PAIRED_DEVICE_TYPE_ACCESSORY - 1), 0,
                            0);
        if (err != ESP_OK) {
            return err;
        }
 ...
 * @details 1 or more interrupts in 100ms is considered clear, 0 with the pin
 * low is asleep, and 0 with the pin high is obstructed. When the obstruction
 * state changes an event of GDO_EVENT_OBST is queued.
 */
static void obst_timer_cb(void* arg) {
    gdo_obstruction_stats_t* stats = (gdo_obstruction_stats_t*) arg;
 ...
 * @details Optional, tiggers on a 50ms interval timer on the preconfigured
 * output pin. The callback is tasked with generating a 50ms square wave pulse
 * on the obst test pin. The pulse is used to test the obstruction sensor
 */
...
static void door_position_sync_timer_cb(void* arg) {
    ...
    float direction_ms =
        g_status.door == GDO_DOOR_STATE_OPENING ? -g_status.open_ms : g_status.close_ms;
 ...
 */
static void v1_status_timer_cb(void* arg) {
    const char secplus1_cmds[] = {0x35, 0x35, 0x35, 0x35, 0x33, 0x33, 0x53, 0x53, 0x38, 0x3A,
...
/**
 * @brief dry-contact toggle a pin. This function will set a pin to level 1 and
 * schedule a reset to zero
 ...
 * fails.
 */
static esp_err_t queue_command(gdo_command_t command, uint8_t nibble, uint8_t byte1,
                               uint8_t byte2) {
    if (!gdo_tx_queue) {
        return ESP_ERR_INVALID_STATE;
    }
 ...
 * @return ESP_OK on success, other non-zero errors from the UART or s/w serial
 * driver.
 */
...
static void decode_v1_packet(uint8_t* packet) {
    ...
    // check that we are not receiving echo back of our sequence of sync status
    // commands / wall plate emulation
    if ((cmd == V1_CMD_QUERY_DOOR_STATUS && g_status.synced) ||
        ((cmd == V1_CMD_QUERY_DOOR_STATUS && !g_status.synced) &&
         resp != V1_CMD_QUERY_OTHER_STATUS)) {
    ...
    } else if ((cmd == V1_CMD_QUERY_OTHER_STATUS && g_status.synced) ||
               ((cmd == V1_CMD_QUERY_OTHER_STATUS && !g_status.synced) &&
                resp != V1_CMD_QUERY_DOOR_STATUS && resp != V1_CMD_QUERY_OTHER_STATUS &&
                resp != V1_CMD_OBSTRUCTION)) {
        update_light_state((gdo_light_state_t) ((resp >> 2) & 1));
        update_lock_state((gdo_lock_state_t) ((~resp >> 3) & 1));
    } else if ((cmd == V1_CMD_OBSTRUCTION && g_status.synced) ||
               ((cmd == V1_CMD_OBSTRUCTION && !g_status.synced) &&
                resp != V1_CMD_QUERY_DOOR_STATUS)) {
        ...
        ESP_LOGD(TAG, "Unhandled command: %02x, resp: %02x, synced: %d", cmd, resp,
                 g_status.synced);
    }
}
...
static void decode_packet(uint8_t* packet) {
    ...
    if ((fixed & 0xFFFFFFFF) == g_status.client_id) { // my commands
        ESP_LOGE(TAG, "received mine: rolling=%07" PRIx32 " fixed=%010" PRIx64 " data=%08" PRIx32,
                 rolling, fixed, data);
        return;
    } else {
        ESP_LOGD(TAG, "received rolling=%07" PRIx32 " fixed=%010" PRIx64 " data=%08" PRIx32,
                 rolling, fixed, data);
    }
    ...
    ESP_LOGI(TAG, "cmd=%03x (%s) byte2=%02x byte1=%02x nibble=%01x", cmd, cmd_to_string(cmd), byte2,
             byte1, nibble);
    ...
    if (cmd == GDO_CMD_STATUS) {
        ...
        update_button_state((gdo_button_state_t) ((byte1 & 1) == 1) ? GDO_BUTTON_STATE_PRESSED
    ...
    } else if ((g_status.door == GDO_DOOR_STATE_OPEN || g_status.door == GDO_DOOR_STATE_CLOSING) &&
               cmd == GDO_CMD_OBST_1) {
        ...
        if (xQueueReceive(gdo_event_queue, (void*) &event, (TickType_t) portMAX_DELAY)) {
                ...
                if (g_status.protocol == GDO_PROTOCOL_SEC_PLUS_V2) {
                    if (!rx_pending) {
                        ...
                        if (rx_packet_size >= GDO_PACKET_SIZE) {
                            // If we have a usable packet then we should process it incase we
                    ...
                    if (rx_packet_size < GDO_PACKET_SIZE) {
                        ESP_LOGW(TAG, "Ignoring RX packet size %u, pending: %u", rx_packet_size,
                                 rx_pending);
                    ...
                    if (rx_packet_size > GDO_PACKET_SIZE) {
                        ESP_LOGW(TAG, "Oversized packet received: %u bytes, messages pending: %u",
                    ...
                    int bytes_read = uart_read_bytes(g_config.uart_num, rx_buffer + rx_buf_index,
                                                     rx_packet_size, 0);
                    if (bytes_read < 0) {
                    ...
                    if (rx_buf_index >= GDO_PACKET_SIZE) {
                        ...
                        if (odd) {
                            if (rx_buffer[0] <= V1_CMD_MIN || rx_buffer[0] >= V1_CMD_MAX) {
                                ...
                                // Decrement the buffer index so we don't over read the buffer
                                // in the loop
                                --rx_buf_index;
                            }
                        }
                        ...
                        // if we decremented the buffer index then we need to move the last
                        // byte to the start
            ...
            case GDO_EVENT_TX_PENDING: {
                ...
                if (now - last_tx_time < g_tx_delay_ms) {
                    ESP_LOGD(TAG, "TX pending, waiting, %" PRIu32 "ms since last TX",
                             now - last_tx_time);
                    err = schedule_event(GDO_EVENT_TX_PENDING,
                                         (g_tx_delay_ms - (now - last_tx_time)) * 1000);
                    if (err != ESP_OK) {
                        ESP_LOGE(TAG, "Failed to schedule TX pending event, %s",
                                 esp_err_to_name(err));
                    }
                    break;
                }
                ...
                if ((g_status.protocol != GDO_PROTOCOL_DRY_CONTACT) &&
                    (rx_pending || gpio_get_level(g_config.uart_rx_pin))) {
                    // If not synced yet just delete the message as the sync loop will
                    // resend it
                    if (!g_status.synced) {
                ...
                if (err != ESP_OK) {
                    ...
                    if ((g_status.protocol & GDO_PROTOCOL_SEC_PLUS_V1) &&
                        (tx_message.cmd == (gdo_command_t) V1_CMD_QUERY_DOOR_STATUS ||
                         tx_message.cmd == (gdo_command_t) V1_CMD_QUERY_OTHER_STATUS ||
                         tx_message.cmd == (gdo_command_t) V1_CMD_OBSTRUCTION)) {
                        // Log verbose if query or obstruction request, as they are sent as
                        // part of wall panel emulation and would fill up the log
...
static void update_door_state(const gdo_door_state_t door_state) {
    ...
    if (!g_status.open_ms) {
        ...
        if (door_state == GDO_DOOR_STATE_OPEN && g_status.door == GDO_DOOR_STATE_OPENING &&
            start_opening != 0) {
    ...
    if (!g_status.close_ms) {
        ...
        if (door_state == GDO_DOOR_STATE_CLOSED && g_status.door == GDO_DOOR_STATE_CLOSING &&
            start_closing != 0) {
    ...
    if (door_state == GDO_DOOR_STATE_OPENING || door_state == GDO_DOOR_STATE_CLOSING) {
        if (g_status.door_position >= 0 && g_status.close_ms > 0 && g_status.open_ms > 0 &&
            !esp_timer_is_active(door_position_sync_timer)) {
    ...
    if ((door_state != g_status.door) || (previous_door_position != g_status.door_position) ||
...
inline static void handle_light_action(gdo_light_action_t light_action) {
    ...
    case GDO_LIGHT_ACTION_TOGGLE:
        if (light_state < GDO_LIGHT_STATE_MAX) {
            light_state =
                light_state == GDO_LIGHT_STATE_OFF ? GDO_LIGHT_STATE_ON : GDO_LIGHT_STATE_OFF;
        }
        break;
    default:
...
static void IRAM_ATTR gdo_contact_isr_handler(void* arg) {
    ...
    if (info->inDebounce) {
        ...
        // keep track that we are notifying task of interrupt (and therefore
        // entering debounce period)
...
static void contact_debounce_timer_cb(void* arg) {
    ...
    if (pinLevel == info->level) {
        ...
        ESP_LOGD(TAG,
                 "Level of pin %d (%s) is %d. Interrupts during debounce: %d "
                 "(%ld.%03ldms)",
                 info->pin, (info->pin == g_config.dc_open_pin) ? "open sensor" : "closed sensor",
                 pinLevel, count, bounceDuration / 1000UL, bounceDuration % 1000UL);
        ...
        portEXIT_CRITICAL(&info->mux);
        ESP_LOGD(TAG,
                 "Debounce of pin %d (%s) not stable, wait for another %dms. "
                 "Interrupts: %d (%ld.%03ldms)",
                 info->pin, (info->pin == g_config.dc_open_pin) ? "open sensor" : "closed sensor",
                 g_config.dc_debounce_ms, count, bounceDuration / 1000UL, bounceDuration % 1000UL);
...
static void gdo_contact_task(void* arg) {
    ...
    ESP_LOGI(TAG, "Initialize dry contact ISR and Task for pin: %d (%s)", info->pin,
    ...
    esp_timer_create_args_t timer_args = {.callback = contact_debounce_timer_cb,
                                          .arg = info,
                                          .dispatch_method = ESP_TIMER_TASK,
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG,
                 "Failed to create contact debounce timer for GPIO pin %d, "
                 "terminating task",
                 info->pin);
        return;
    }
    ...
    // Notify ourselves... so we will immediately process current GPIO level to
    // set initial state
    info->level = gpio_get_level(info->pin);
    ...
    for (;;) {
        ...
        ESP_LOGD(TAG, "Wake up for GPIO pin %d (%s), level is %d", info->pin,
                 (info->pin == g_config.dc_open_pin) ? "open sensor" : "closed sensor",
                 info->level);
...
/**
 * @brief determine door state (open/opening/closing/closed) from dry contact
 * states
 */
void decode_dry_contact(gdo_contact_type_t contact, uint32_t level) {
    ...
    if (g_status.protocol == GDO_PROTOCOL_DRY_CONTACT) {
        ...
        // are using Sec+ 1.0 or Sec+ 2.0 door control type. You must call
        // gdo_init() with values set for dc_open_pin and dc_close_pin in the config
        // struct.
        if (dryContactDoorOpen) {
            gdo_door_open();
            dryContactDoorOpen = false;
