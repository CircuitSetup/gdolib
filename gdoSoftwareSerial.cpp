...
public:
  ...
  uint8_t *fetch_buf(void) { return m_rx_buf; }
};
...
esp_err_t serial_start(QueueHandle_t rxQueue, MessageBufferHandle_t *txBuffer,
                       TaskHandle_t *serialTask, gpio_num_t rxPin,
                       gpio_num_t txPin) {
  // rxQueue is passed into us, that is where we will send received packets
  // txBuffer we will create and return, that is where packets to transmit will
  // be sent to us
  ...
  *txBuffer = xMessageBufferCreate(sizeof(serial_transmit_t) + 8);
  ...
  // High priority as it needs to handle in real time, pin to CPU 1 so does not
  // share with HomeKit/WiFi/mdns/etc.
#ifdef CONFIG_FREERTOS_UNICORE
  if (xTaskCreate((TaskFunction_t)serial_task, "serial_task", 4096,
                  &serial_task_params, 15, &serial_task_handle) != pdPASS)
#else
  if (xTaskCreatePinnedToCore((TaskFunction_t)serial_task, "serial_task", 4096,
                              &serial_task_params, 15, &serial_task_handle,
                              1) != pdPASS)
#endif
  {
    serial_stop();
  ...
  *serialTask = serial_task_handle;
  ...
  // to read. This avoids need to have task spin in a tight loop polling for
  // data.
...
esp_err_t serial_set_protocol(gdo_protocol_type_t protocol) {
  ...
  ESP_LOGI(TAG, "Set serial port protocol to %s",
  ...
  if (protocol == GDO_PROTOCOL_SEC_PLUS_V2) {
    ...
    swSerial.enableAutoBaud(
        true); // found in ratgdo/espsoftwareserial branch autobaud
  } else {
...
void serial_task(TaskParams *arg) {
  ...
  for (;;) {
    while (swSerial.available()) {
      ...
      if (serial_protocol == GDO_PROTOCOL_SEC_PLUS_V2) {
        ...
        if (!readingMsg) {
          ...
          if (gotMessage == false &&
              (esp_timer_get_time() - last_rx) > (100 * 1000)) {
            ESP_LOGW(TAG, "RX message timeout");
            // if we have a partial packet and it's been over 100ms since last
            // byte was read, the rest is not coming (a full packet should be
            // received in ~20ms), discard it so we can read the following
            // packet correctly
            readingMsg = false;
            byteCount = 0;
          }
    ...
    // We have read all data from serial port.  Now look and see if there is a
    // packet to send.
    if (xMessageBufferReceive(arg->txBuffer, &txPacket, sizeof(txPacket), 0)) {
      ...
      if (txPacket.size > 1) {
        ...
        // check to see if anyone else is continuing to assert the bus after we
        // have released it
        if (gpio_get_level(arg->rxPin)) {
          err = ESP_FAIL;
          ESP_LOGE(TAG, "Collision detected trying to send packet");
          // There is a test for this in the sending gdolib code and it
          // requeues, so this may never occur.
        } else {
          swSerial.enableRx(false); // Disable RX so our own packet is not
        ...
        if (gpio_get_level(arg->rxPin)) {
          ...
          bool poll_cmd = (*txPacket.packet == 0x38) ||
                          (*txPacket.packet == 0x39) ||
                          (*txPacket.packet == 0x3A);
