...
esp_err_t serial_start(QueueHandle_t rxQueue, MessageBufferHandle_t* txBuffer,
    ...
    *txBuffer = xMessageBufferCreate(sizeof(serial_transmit_t) + 8);
    ...
    if (xTaskCreate((TaskFunction_t) serial_task, "serial_task", 4096, &serial_task_params, 15,
                    &serial_task_handle) != pdPASS)
#else
    if (xTaskCreatePinnedToCore((TaskFunction_t) serial_task, "serial_task", 4096,
                                &serial_task_params, 15, &serial_task_handle, 1) != pdPASS)
#endif
    {
        serial_stop();
        return ESP_ERR_NO_MEM;
        ...
        if (xMessageBufferReceive(arg->txBuffer, &txPacket, sizeof(txPacket), 0)) {
            ...
            if (txPacket.size > 1) {
                ...
                if (gpio_get_level(arg->rxPin)) {
                    ...
                    bool poll_cmd = (*txPacket.packet == 0x38) || (*txPacket.packet == 0x39) ||
