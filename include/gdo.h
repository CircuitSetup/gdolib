...
typedef struct {
  ...
  bool toggle_only;      // Used when the door opener only supports the toggle
                         // command.
  bool tof_timer_active; // ToF interval timer active
  bool obst_test_pulse_timer_active; // Obstruction test pulse output pin timer
                                     // active
  ...
  int32_t
      door_target; // Door target position in percentage (0-10000) [OPEN-CLOSED]
...
typedef struct {
  ...
  uint32_t
      dc_debounce_ms; // dry contact debounce timer duration in milliseconds.
 ...
 */
typedef void (*gdo_event_callback_t)(const gdo_status_t *status,
                                     gdo_cb_event_t event, void *user_arg);
